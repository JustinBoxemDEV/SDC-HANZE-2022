...
void CaptureScreenLinux::initimage(struct shmimage* image) {
    image->ximage = NULL;
    image->shminfo.shmaddr = (char*) -1;
}

void CaptureScreenLinux::destroyimage(Display* dsp, struct shmimage* image) {
    if (image->ximage) {
        (dsp, &image->shminfo);
...
int CaptureScreenLinux::createimage(Display* dsp, struct shmimage* image, int width, int height) {
...
void CaptureScreenLinux::getrootwindow(Display* dsp, struct shmimage* image) {
    XShmGetImage(dsp, XDefaultRootWindow(dsp), image->ximage, 0, 0, AllPlanes);
}

long CaptureScreenLinux::timestamp() {
   struct timeval tv;
   struct timezone tz;
   gettimeofday(&tv, &tz);
...
Window CaptureScreenLinux::createwindow(Display* dsp, int width, int height) {
...
void CaptureScreenLinux::destroywindow(Display* dsp, Window window) {
    XDestroyWindow(dsp, window);
}

unsigned int CaptureScreenLinux::getpixel(struct shmimage* src, struct shmimage* dst, int j, int i, int w, int h) {
    int x = (float)(i* src->ximage->width) / (float)w;
    int y = (float)(j* src->ximage->height) / (float)h;
    return src->data[y* src->ximage->width + x];
}

int CaptureScreenLinux::processimage(struct shmimage* src, struct shmimage* dst) {
...
int CaptureScreenLinux::load(Display* dsp, Window window, struct shmimage* src, struct shmimage* dst) {
    XGCValues xgcvalues;
    ...
    while (running) {
        while (XPending(dsp)) {
            ...
            if((xevent.type == ClientMessage && xevent.xclient.data.l[0] == delete_atom) || xevent.type == DestroyNotify) {
                running = false;
                break;
            } else if(xevent.type == ConfigureNotify) {
                if (xevent.xconfigure.width == dstwidth && xevent.xconfigure.height == dstheight) {
                    initialized = true;
                }
            }
        }
