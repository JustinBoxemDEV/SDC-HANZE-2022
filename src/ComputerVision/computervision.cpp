...
std::vector<int> ComputerVision::Histogram(cv::Mat src, int threshold)
{
    std::vector<int> points;
    cv::Rect roi(0, threshold, src.cols, src.rows - threshold);

    cv::Mat img = src(roi);

    for (int i = 0; i < img.cols; i++)
    {
        points.push_back(cv::countNonZero(img.col(i)));
    }
    return points;
}
...
cv::Mat ComputerVision::InterpolatedThresholding(int thresholdUpper, int thresholdLower, cv::Mat src, int type){
    int thresholdDelta = thresholdLower - thresholdUpper;
    cv::Mat thresholdedResult = cv::Mat::zeros(src.size(), type);
    for (int y = 0; y < src.rows; y++){
        int threshold = thresholdUpper + thresholdDelta * y / src.rows;
        for (int x = 0; x < src.cols; x++)
        {
            double color = src.at<double>(cv::Point(x,y)) ; 
            if(color >= threshold){
...
cv::Mat ComputerVision::CreateBinaryImage(cv::Mat src)
{
    ...
    sobely = InterpolatedThresholding(150, 85, sobely, CV_64F);
    ...
    // hsvChannels[1] = InterpolatedThresholding(200, 105, hsvChannels[1], CV_8SC1);
    // imshow("hsv s'", hsvChannels[1]);
    ...
    // hlsChannels[2] = InterpolatedThresholding(150, 35, hlsChannels[2], CV_16S);
    // imshow("hls s'", hlsChannels[2]);
    ...
    // hlsChannels[1] = InterpolatedThresholding(255, 180, hlsChannels[1]);
...
void ComputerVision::PredictTurn(cv::Mat src)
{
    ...
    leftMaxX = std::min(leftMaxX, int(src.cols * 0.5));
    rightMaxX = std::max(rightMaxX, int(src.cols * 0.5));
    ...
    for (int x = 0; x < warped.rows; x++)
    {
        ...
        positionR.y = x;
        positionR.x = (fitR[2] * pow(x, 2) + (fitR[1] * x) + fitR[0] + ((approximateRight) ? 400 : 0));

        cv::Point2f positionL;
        positionL.y = x;
        positionL.x = (fitL[2] * pow(x, 2) + (fitL[1] * x) + fitL[0] - ((approximateLeft) ? 400 : 0));

        int laneLeft = (fitL[2] * pow(x, 2) + (fitL[1] * x) + fitL[0] - ((approximateLeft) ? 400 : 0));
        int laneRight = (fitR[2] * pow(x, 2) + (fitR[1] * x) + fitR[0] + ((approximateRight) ? 400 : 0));
    ...
    int laneLeft = (fitL[2] * pow(src.rows, 2) + (fitL[1] * src.rows) + fitL[0] - ((approximateLeft) ? 400 : 0));
    int laneRight = (fitR[2] * pow(src.rows, 2) + (fitR[1] * src.rows) + fitR[0] + ((approximateRight) ? 400 : 0));
